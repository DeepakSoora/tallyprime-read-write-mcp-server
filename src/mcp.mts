...
import { parseExcelFile, PurchaseVoucherData } from './excel.mjs';
...
IMPORTANT: Validate ledger and stock item names using list-master tool before importing.`,
    ...
    async (args) => {
      ...
      const result = await handleBatchPush(
        'purchase-entry-batch',
        preparedVouchers,
        batchSize,
        args.targetCompany
      );
  ...
  mcpServer.registerTool(
    ...
    async (args) => {
      ...
      const totalAmount = args.debitEntries.reduce((sum: number, entry: any) => sum + entry.amount, 0);
  ...
  mcpServer.registerTool(
    ...
    async (args) => {
      ...
      const totalAmount = args.creditEntries.reduce((sum: number, entry: any) => sum + entry.amount, 0);
      inputParams.set('totalAmount', totalAmount);
  ...
  mcpServer.registerTool(
    ...
    async (args) => {
      ...
      const totalDebit = args.debitEntries.reduce((sum: number, e: any) => sum + e.amount, 0);
      const totalCredit = args.creditEntries.reduce((sum: number, e: any) => sum + e.amount, 0);
  ...
  mcpServer.registerTool(
    ...
    async (args) => {
      ...
      if (args.mode === 'invoice') {
        ...
        let inventoryTotal = args.inventoryEntries!.reduce((sum: number, item: any) => sum + (item.quantity * item.rate), 0);
        let ledgerTotal = args.ledgerEntries?.reduce((sum: number, entry: any) => sum + entry.amount, 0) || 0;
        inputParams.set('invoiceTotal', inventoryTotal + ledgerTotal);
      }
